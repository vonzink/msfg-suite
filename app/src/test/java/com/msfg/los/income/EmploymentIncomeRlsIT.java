...
/**
 * DB-layer RLS isolation for employment, income_item, and income_verification tables.
 *
 * Mirrors the mechanism in RlsIT (tenancy package):
 ...
 *  - Asserts fail-closed (RESET GUC → count 0) on employment.
 */
class EmploymentIncomeRlsIT extends AbstractIntegrationTest {

    @Autowired DataSource ds;
    @Autowired JdbcTemplate jdbc;

    // Distinct UUIDs — no overlap with RlsIT (c1/c2) or any other test
    static final String ORG_X = "00000000-0000-0000-0000-0000000000d1";
    static final String ORG_Y = "00000000-0000-0000-0000-0000000000d2";
    ...
    void seedOrgsAndLoans() {
        ...
        for (String[] o : new String[][]{{ORG_X, "org-income-x"}, {ORG_Y, "org-income-y"}})
            jdbc.update(
        ...
        jdbc.update(
            ...
            loanIdX, "RLS-INC-X-" + loanIdX.toString().substring(0, 8),
            UUID.randomUUID(), ORG_X);
        jdbc.update(
            "insert into loan (id,version,loan_number,loan_officer_id,status,org_id) " +
            "values (?,0,?,?,'STARTED',?::uuid)",
            loanIdY, "RLS-INC-Y-" + loanIdY.toString().substring(0, 8),
            UUID.randomUUID(), ORG_Y);
    }

    @Test
    void rlsIsolatesEmploymentIncomeVerificationByOrg() throws Exception {
        try (Connection c = ds.getConnection()) {
            ...
            try {
                ...
                // employment (FK to borrower_party; loan_id is plain uuid — random OK)
                UUID employmentIdX = UUID.randomUUID();
                try (var ps = c.prepareStatement(
                        "insert into employment (id,version,org_id,loan_id,borrower_id,ordinal) " +
                        "values (?,0,?::uuid,?,?,0)")) {
                    ps.setObject(1, employmentIdX);
                    ps.setString(2, ORG_X);
                ...
                // income_item (FK to borrower_party + optional FK to employment)
                UUID incomeItemIdX = UUID.randomUUID();
                try (var ps = c.prepareStatement(
                        "insert into income_item " +
                        "(id,version,org_id,loan_id,borrower_id,employment_id,income_type,monthly_amount,ordinal) " +
                        "values (?,0,?::uuid,?,?,?,'BASE',1000,0)")) {
                    ps.setObject(1, incomeItemIdX);
                    ps.setString(2, ORG_X);
                    ps.setObject(3, UUID.randomUUID()); // plain loan_id
                    ps.setObject(4, borrowerIdX);
                    ps.setObject(5, employmentIdX);
                    ps.executeUpdate();
                }

                // income_verification (no borrower FK required — borrower_id nullable)
                UUID verificationIdX = UUID.randomUUID();
                try (var ps = c.prepareStatement(
                        "insert into income_verification " +
                        "(id,version,org_id,loan_id,verification_type,status) " +
                        "values (?,0,?::uuid,?,'VOI','ORDERED')")) {
                ...
                assertThat(countTable(c, ORG_Y, "employment")).isZero();
                assertThat(countTable(c, ORG_Y, "income_item")).isZero();
                assertThat(countTable(c, ORG_Y, "income_verification")).isZero();

                // ── ORG_X sees its own rows (exactly 1 each — orgs are fresh) ────────
                assertThat(countTable(c, ORG_X, "employment")).isEqualTo(1);
                assertThat(countTable(c, ORG_X, "income_item")).isEqualTo(1);
                assertThat(countTable(c, ORG_X, "income_verification")).isEqualTo(1);

                // ── Fail-closed: RESET GUC → count 0 (deny-all) ─────────────────────
                setOrg(c, null); // RESET app.current_org
                try (var st = c.createStatement();
                     var rs = st.executeQuery("select count(*) from employment")) {
                    rs.next();
