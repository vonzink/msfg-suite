...
class AssetVerificationIT extends AbstractIntegrationTest {
    ...
    // --- happy path: VOA → 201 with stub result ---

    @Test
    void orderVoaReturns201WithStubResult() throws Exception {
        String loanId = createLoan();

        mvc.perform(post("/api/loans/{loanId}/assets/verifications", loanId).with(lo())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"verificationType\":\"VOA\"}"))
    ...
    void listAfterOneOrderReturnsLengthOne() throws Exception {
        ...
        mvc.perform(get("/api/loans/{loanId}/assets/verifications", loanId).with(lo()))
    ...
    void crossOrgPostReturns404() throws Exception {
        ...
        mvc.perform(post("/api/loans/{loanId}/assets/verifications", loanId).with(otherOrg)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"verificationType\":\"VOA\"}"))
                .andExpect(status().isNotFound());
    }

    // --- foreign borrowerId (from a different loan) → 400 ---
    ...
    void orderWithForeignBorrowerReturns400() throws Exception {
        ...
        mvc.perform(post("/api/loans/{loanId}/assets/verifications", l1).with(lo())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"verificationType\":\"VOA\",\"borrowerId\":\"%s\"}".formatted(b2)))
        ...
        mvc.perform(post("/api/loans/{loanId}/assets/verifications", l1).with(lo())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"verificationType\":\"VOA\",\"borrowerId\":\"%s\"}".formatted(b1)))
                .andExpect(status().isCreated());
    }
    ...
    void noTokenReturns401() throws Exception {
        mvc.perform(post("/api/loans/{loanId}/assets/verifications", UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"verificationType\":\"VOA\"}"))
