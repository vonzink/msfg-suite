...
// JpaSpecificationExecutor is mixed in so the pipeline (Phase 2 T4) pushes every filter facet into
// SQL via LoanSpecifications (query-side, never load-all-then-filter — same pattern as documents).
